        ...
        for item in azure_ips["values"]:
            for prefix in item["properties"]["addressPrefixes"]:
                if target_ip in IPNetwork(str(prefix)):
                    matched = True
                    logger.info(
                        f'Match for Azure range "{prefix}", region "{item["properties"]["region"]}" and '
...
def match_gcp(target_ip):
    ...
    try:
        ...
        for item in gcp_ips["prefixes"]:
            if target_ip in IPNetwork(
                str(item.get("ipv4Prefix", item.get("ipv6Prefix")))
            ):
                matched = True
                logger.info(
                    f'Match for GCP range "{item.get("ipv4Prefix", item.get("ipv6Prefix"))}", '
        ...
        for region in oci_ips["regions"]:
            for cidr_item in region["cidrs"]:
                if target_ip in IPNetwork(str(cidr_item["cidr"])):
                    matched = True
                    logger.info(
                        f'Match for OCI range "{cidr_item["cidr"]}", region "{region["region"]}" and '
    ...
    try:
        ...
        for item in do_ips:
            if target_ip in IPNetwork(item["range"]):
                matched = True
                logger.info(
                    f'Match for DigitalOcean range "{item["range"]}", country "{item["country"]}", '
...
def main():
    ...
    parser.add_argument("ip", help="The IP to evaluate, e.g.: 8.8.8.8")
